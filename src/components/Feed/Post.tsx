...
function Post({post}: Props) {
  ...
  //post
  const {author, imagePath, comments, content, countLikes, countReTweet} = post;
  const displayname = `${author.firstName} ${author.lastName}`;
  const {width: windowWidth} = useViewport();
  const width = windowWidth < 645 ? "80%" : (imagePath && imagePath?.length > 1 ? 200 : 400);
  ...
  const sendComment = () => {
    if (text.length > 0) {
      addComment(post._id, text).then((res) => {
        setText("");
  ...
  const actions = <div className="post__footer">
    <span className='custom-comment-action' key='reply' onClick={() => setShowComment(!showComment)}>
      {showComment ?
        <Comments theme="two-tone" size="21" fill={['#333', '#4a90e2']} strokeWidth={3}/>
        :
    ...
    >
      {like ? <ThumbsUp theme="two-tone" size="21" fill={['#333', '#2F88FF']} strokeWidth={3}/> :
    ...
    <span
      ...
      onClick={() => setRe(!re)}
    >
      {re ? <ShareTwo theme="filled" size="21" fill="#2f88ff" strokeWidth={3}/> :
      ...
      >
        {showComment && comments &&
          comments.map((comment, index) => (
            <Comment
              ...
              avatar={
                <Avatar>
                  {comment.author.avatar ? <img src={comment.author.avatar} alt={comment.author.username}/> :
                    <IconUser/>}
                </Avatar>
              }
              author={
                <div className="post__header">
                  <h5
                    style={{margin: 0, marginRight: 4}}>{`${comment.author.firstName} ${comment.author.lastName}`}</h5>
        ...
        {showComment &&
            <Comment
                ...
                align='right'
                actions={[
                  <Button key='0' type='primary' shape='round' onClick={sendComment}>Reply</Button>,
                ]}
                avatar={<Avatar>
